...
This function takes a line and an ending number, and chooses between 1 and `iRandomEnd` variations on the line by appending `_x` (`x` being the generated number) after the line name. This function runs `aivl_play_ext`, initialising the non-used arguments to:
- li_script_start = -1
- li_script_end = -1
- interrupt = 1
