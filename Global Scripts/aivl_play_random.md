...
This function takes a line and an ending number, and chooses between 1 and `iRandomEnd` variations on the line by appending `_x` (`x` being the generated number) after the line name. This is identical to `aivl_play_interrupt()` aside from setting `interrupt` to 2, instead of 1. Unused arguments for `aivl_play_ext` are set to:
- li_script_start = -1
- li_script_end = -1
- interrupt = 0
